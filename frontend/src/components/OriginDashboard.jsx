...
/* ─── Module-level OSRM route geometry cache ─────────────────────── */
var ROAD_GEOMETRY_CACHE = {};

function getRouteColor(delayProb, isSelected) {
  if (isSelected)      { return '#3B82F6'; } // Distinct Blue replacing dull green
...
/* ═══════════════════════════════════════════════════════════════════
   LiveRouteMap — Clean Light Theme Map
   ═══════════════════════════════════════════════════════════════════ */
    ...
    <div style={{ position: 'relative', borderRadius: '12px', overflow: 'hidden', border: '1px solid #E5E4DE', boxShadow: '0 4px 12px rgba(0,0,0,0.04)' }}>
      {loadingRoutes && (
        <div style={{ position: 'absolute', top: '12px', right: '12px', zIndex: 1100, background: 'rgba(255,255,255,0.95)', border: '1px solid #E5E4DE', borderRadius: '8px', padding: '6px 12px', display: 'flex', alignItems: 'center', gap: '8px', boxShadow: '0 2px 8px rgba(0,0,0,0.05)' }}>
          ...
          <span style={{ fontFamily: 'DM Mono, monospace', fontSize: '10px', color: '#64748b', letterSpacing: '0.06em' }}>LOADING ROAD DATA…</span>
        </div>
      )}
        ...
        {[['#3B82F6','OPTIMAL / ACTIVE'],['#10B981','SAFE'],['#F59E0B','MODERATE'],['#EF4444','DANGER']].map(function (pair) {
          return (
            <div key={pair[1]} style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
              ...
              <span style={{ fontFamily: 'DM Mono, monospace', fontSize: '9px', color: '#64748B', letterSpacing: '0.08em', fontWeight: 600 }}>{pair[1]}</span>
            </div>
          );
        })}
      </div>
...
/* ═══════════════════════════════════════════════════════════════════
   RouteConfigPanel — White UI Design
   ═══════════════════════════════════════════════════════════════════ */
function RouteConfigPanel({ sourceNode, destNode, onSourceChange, onDestChange, activePath }) {
  ...
  return (
    <Card style={{ padding: '24px', boxShadow: '0 4px 12px rgba(0,0,0,0.02)' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '20px' }}>
        <Navigation size={16} color="#3B82F6" />
        <span style={{ fontFamily: 'DM Mono, monospace', fontSize: '11px', color: '#111827', letterSpacing: '0.1em', textTransform: 'uppercase', fontWeight: 600 }}>Routing Parameters</span>
      </div>
        ...
        <div>
          ...
          <div style={{ position: 'relative' }}>
            <motion.select whileFocus={{ borderColor: '#3B82F6', background: '#FFFFFF', boxShadow: '0 0 0 3px rgba(59,130,246,0.1)' }}
              value={sourceNode} onChange={function (e) { onSourceChange(e.target.value); }} style={selectStyle}>
              {ALL_NODES.map(n => <option key={n.id} value={n.id}>{n.name}</option>)}
            </motion.select>
        ...
        <div>
          ...
          <div style={{ position: 'relative' }}>
            <motion.select whileFocus={{ borderColor: '#8B5CF6', background: '#FFFFFF', boxShadow: '0 0 0 3px rgba(139,92,246,0.1)' }}
              value={destNode} onChange={function (e) { onDestChange(e.target.value); }} style={selectStyle}>
              {ALL_NODES.map(n => <option key={n.id} value={n.id}>{n.name}</option>)}
            </motion.select>
      ...
      <div style={{ padding: '12px 16px', background: '#F0FDF4', border: '1px solid #D1FAE5', borderRadius: '8px', display: 'flex', alignItems: 'center', gap: '8px' }}>
        ...
        <span style={{ fontFamily: 'DM Mono, monospace', fontSize: '11px', color: '#065F46', letterSpacing: '0.04em', fontWeight: 500 }}>
...
function ThreatPanel({ threatened, loading, submitted, submitError, onSimulate, onReset, resetting, threatText, setThreatText, targetNode, setTargetNode }) {
  ...
  var labelStyle = { fontFamily: 'DM Mono, monospace', fontSize: '10px', color: '#64748B', letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: '8px', display: 'block', fontWeight: 500 };
  var targetableNodes = ALL_NODES.filter(n => n.role !== 'Origin');

  return (
    <Card style={{ padding: '24px', borderColor: threatened ? '#FCA5A5' : '#E5E4DE', boxShadow: threatened ? '0 4px 16px rgba(239,68,68,0.1)' : '0 4px 12px rgba(0,0,0,0.02)', display: 'flex', flexDirection: 'column', gap: '20px' }}>
      
      <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
        <AlertTriangle size={16} color={threatened ? '#EF4444' : '#111827'} />
        <span style={{ fontFamily: 'DM Mono, monospace', fontSize: '11px', color: threatened ? '#EF4444' : '#111827', letterSpacing: '0.1em', textTransform: 'uppercase', fontWeight: 600 }}>
      ...
      <div>
        ...
        <div style={{ position: 'relative' }}>
          <motion.select whileFocus={{ borderColor: '#EF4444', background: '#FFFFFF', boxShadow: '0 0 0 3px rgba(239,68,68,0.1)' }}
            value={targetNode} onChange={e => setTargetNode(e.target.value)} style={{ ...inputStyle, cursor: 'pointer', appearance: 'none', WebkitAppearance: 'none' }}>
            <option value="">— Select Target Zone —</option>
            {targetableNodes.map(n => <option key={n.id} value={n.name}>{n.name}</option>)}
          </motion.select>
      ...
      <div>
        ...
        <motion.textarea whileFocus={{ borderColor: '#3B82F6', background: '#FFFFFF', boxShadow: '0 0 0 3px rgba(59,130,246,0.1)' }}
          value={threatText} onChange={e => setThreatText(e.target.value)}
          onKeyDown={e => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); onSimulate(threatText, targetNode); } }}
      ...
      <div style={{ display: 'flex', gap: '12px' }}>
        <motion.button whileHover={{ scale: (loading || !threatText.trim() || !targetNode) ? 1 : 1.02 }} whileTap={{ scale: 0.98 }}
          onClick={() => onSimulate(threatText, targetNode)}
          disabled={loading || !threatText.trim() || !targetNode}
          style={{
            flex: 1, padding: '12px', background: threatened ? '#FEF2F2' : '#EFF6FF',
            border: '1px solid', borderColor: threatened ? '#FECACA' : '#BFDBFE', borderRadius: '8px',
            color: threatened ? '#DC2626' : '#2563EB', fontSize: '13px', fontWeight: 600,
            cursor: (loading || !threatText.trim() || !targetNode) ? 'not-allowed' : 'pointer',
            opacity: (!threatText.trim() || !targetNode) ? 0.5 : 1, fontFamily: 'Space Grotesk, sans-serif',
          }}
        >
          {loading ? 'Simulating...' : 'Commence Attack Protocol →'}
        </motion.button>
        {threatened && (
          <motion.button whileHover={{ scale: 1.02, background: '#F1F5F9' }} whileTap={{ scale: 0.98 }}
...
/* ═══════════════════════════════════════════════════════════════════
   RouteComparisonPanel — High Contrast Table
   ═══════════════════════════════════════════════════════════════════ */
function RouteComparisonPanel({ analysis, isAnalyzing, selectedId, onSelect }) {
  if (isAnalyzing) {
    return (
      <Card style={{ padding: '32px', display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '140px', boxShadow: '0 4px 12px rgba(0,0,0,0.02)' }}>
        <div style={{ width: '16px', height: '16px', borderRadius: '50%', background: '#3B82F6', animation: 'leaflet-pulse 1s infinite' }} />
        <span style={{ marginLeft: '12px', fontFamily: 'DM Mono, monospace', fontSize: '11px', color: '#64748B', letterSpacing: '0.08em' }}>COMPUTING OPTIMAL PATHWAYS...</span>
      </Card>
    );
  }
  ...
  return (
    <Card style={{ padding: '24px', boxShadow: '0 4px 12px rgba(0,0,0,0.02)' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '16px' }}>
        <span style={{ fontFamily: 'DM Mono, monospace', fontSize: '11px', color: '#111827', letterSpacing: '0.1em', textTransform: 'uppercase', fontWeight: 600 }}>
          Route Matrix Calculation
        </span>
        <span style={{ fontFamily: 'DM Mono, monospace', fontSize: '10px', color: '#64748B', background: '#F1F5F9', padding: '4px 8px', borderRadius: '6px' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
        {analysis.routes.map(function (rt) {
          ...
          var pathNames = rt.path.map(n => { var nd = findNode(n); return nd ? nd.name : n; });
          return (
            <motion.div key={rt.id} whileHover={{ scale: 1.005, y: -2 }} onClick={() => onSelect(rt.id)}
              style={{ padding: '16px', background: isSel ? '#EFF6FF' : '#FFFFFF', border: '1px solid', borderColor: isSel ? '#3B82F6' : '#E5E7EB', borderRadius: '10px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'space-between', transition: 'background 0.2s', boxShadow: isSel ? '0 4px 12px rgba(59,130,246,0.1)' : '0 2px 4px rgba(0,0,0,0.02)' }}
            >
              <div style={{ flex: 1 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '6px' }}>
                  {rt.isOptimal && <span style={{ fontFamily: 'DM Mono, monospace', fontSize: '9px', background: '#D1FAE5', color: '#065F46', padding: '3px 8px', borderRadius: '6px', fontWeight: 600 }}>PRIORITY</span>}
              ...
              <div style={{ textAlign: 'right', minWidth: '60px' }}>
                <div style={{ fontSize: '18px', fontWeight: 700, color: riskColor, fontFamily: 'Space Grotesk, sans-serif' }}>
                  {Math.round(rt.delayProbability)}%
                </div>
                <div style={{ fontSize: '9px', color: '#94A3B8', fontFamily: 'DM Mono, monospace', letterSpacing: '0.04em' }}>DELAY</div>
              </div>
            </motion.div>
          );
        })}
...
function OriginDashboard() {
  ...
  function addLog(msg) {
    ...
    setLogs(prev => [{ t, msg }].concat(prev.slice(0, 11)));
  }
  ...
  function hydrateFromState(data) {
    ...
    if (data.active_shipments) {
      setCarriersActive(data.active_shipments.filter(s => s.status === 'in_transit').length);
    }
    if (data.nodes) {
      setNodesOnline(data.nodes.filter(n => n.status === 'active').length);
      ...
      .then(data => {
        if (!cancelled) {
          ...
          if (data && data.routes && data.routes.length > 0) {
            var optimalRoute = data.routes.find(r => r.isOptimal) || data.routes[0];
            setActivePath(optimalRoute.path);
            addLog(`Analysis Matrix complete. Optimal Delay: ${Math.round(optimalRoute.delayProbability)}%.`);
          }
        }
      })
      .catch(e => { if (!cancelled) { setIsAnalyzing(false); addLog('Route bounds error: ' + e.message); }});
  ...
  useEffect(function () {
    fetch('/api/state').then(r => r.json()).then(data => {
      ...
      if (data.threats && data.threats.length > 0) {
        setLogs(data.threats.map(th => ({
          t: new Date(th.timestamp).toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit', hour12: false }),
          msg: `🚨 ${th.city} — ${th.action}`,
        })));
      } else { addLog('Global Systems Online. Network Stable.'); }
    }).catch(() => addLog('Global Systems Online. Network Stable.'));
  }, []);

  useEffect(function () {
    var socket = io('http://localhost:3001', { transports: ['websocket', 'polling'] });
    socket.on('state_updated', hydrateFromState);
    socket.on('route_updated', d => {
      if (d.blockedNode) setBlockedNode(normalizeNode(d.blockedNode));
      var newPathStr = normalizePath(d.newPath || []).map(id => { var n = findNode(id); return n ? n.name : id; }).join(' → ');
      addLog(`🚨 AI HEAL INITIATED — ${d.blockedNode} locked out. New vector: ${newPathStr}`);
      setThreatened(true);
    });
    socket.on('route_reset', () => {
      setThreatened(false); setBlockedNode(null); setActivePath(prev => prev); setThreatCount(0);
      addLog('✓ Network Restored. All nodes unlocked.');
    });
    return () => socket.disconnect();
  }, []);

  /* Powerful GSAP Staggered Cascade Entrance */
  useEffect(function () {
    var ctx = gsap.context(() => {
      gsap.from('.g-stat', { opacity: 0, scale: 0.9, y: 30, stagger: 0.1, duration: 0.8, ease: 'back.out(1.5)' });
      gsap.from('.g-card', { opacity: 0, y: 40, stagger: 0.15, delay: 0.3, duration: 1, ease: 'power3.out' });
    }, contentRef);
    return () => ctx.revert();
  }, []);
  ...
  async function handleSimulate(text, target) {
    ...
    try {
      ...
      if (data.routeResult && data.routeResult.rerouted) {
        setThreatened(true); setSubmitted(true); setTimeout(() => setSubmitted(false), 4000);
      } else {
        setSubmitError((data.aiResult && data.aiResult.reason) ? data.aiResult.reason : 'Discarded by Agent.');
        setTimeout(() => setSubmitError(''), 5000);
      }
    } catch (err) {
      setSubmitError('Connection Timeout.'); setTimeout(() => setSubmitError(''), 5000);
    }
  ...
  return (
    <Layout title="Global Architecture Hub" status={threatened ? 'THREAT ACTIVE' : 'SYSTEM OPTIMAL'} statusOk={!threatened}>
      <div ref={contentRef}>
        ...
        {/* Dynamic Light/White UI Stat Cards */}
        ...
        {/* Lower Tier: Map & Threat/Log Sidebars */}
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 380px', gap: '24px', marginBottom: '24px' }}>
          
          <div className="g-card">
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px', padding: '0 4px' }}>
              <span style={{ fontFamily: 'DM Mono, monospace', fontSize: '11px', color: '#111827', letterSpacing: '0.1em', textTransform: 'uppercase', fontWeight: 600 }}>
                Territorial Operations Map
              </span>
              {threatened && <span className="mono" style={{ fontSize: '10px', color: '#EF4444', background: '#FEF2F2', padding: '4px 10px', borderRadius: '6px', fontWeight: 600, border: '1px solid #FECACA' }}>⚠ ACTIVE REROUTE</span>}
            </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
            <div className="g-card">
              <ThreatPanel threatened={threatened} loading={loading} submitted={submitted} submitError={submitError} onSimulate={handleSimulate} onReset={handleReset} resetting={resetting} threatText={threatText} setThreatText={setThreatText} targetNode={targetNode} setTargetNode={setTargetNode} />
            </div>

            <Card className="g-card" style={{ padding: '24px', flex: 1, display: 'flex', flexDirection: 'column', boxShadow: '0 4px 12px rgba(0,0,0,0.02)' }}>
              <span style={{ fontFamily: 'DM Mono, monospace', fontSize: '11px', color: '#111827', letterSpacing: '0.1em', textTransform: 'uppercase', fontWeight: 600, marginBottom: '16px', display: 'block' }}>
                System Audit Trace
              </span>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', overflowY: 'auto', flex: 1 }}>
                {logs.length === 0 && <p style={{ fontSize: '13px', color: '#94A3B8', fontFamily: 'Space Grotesk' }}>Recording telemetry...</p>}
                {logs.map((log, i) => (
                  <motion.div key={i} initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} transition={{ type: 'spring' }} style={{ display: 'flex', gap: '16px', alignItems: 'baseline', padding: '8px 0', borderBottom: i < logs.length - 1 ? '1px solid #F1F5F9' : 'none' }}>
